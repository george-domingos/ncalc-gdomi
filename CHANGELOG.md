...
* Function and parameter callback delegates and argument types now use unified types in `NCalc.Handlers`. `FunctionArgs`, `ParameterArgs`, `ExpressionFunctionData`, `ExpressionParameterData`, `AsyncFunctionArgs`, `AsyncParameterArgs`, `AsyncExpressionFunctionData`, and `AsyncExpressionParameterData` were replaced by `FunctionEventArgs` and `ParameterEventArgs`.
* Function callback arguments no longer expose arrays of nested `Expression` or `AsyncExpression` instances. `FunctionEventArgs` exposes `LogicalExpression` arguments and provides `Evaluate(index)` / `EvaluateAsync(index)` helpers for evaluating them with the current `ExpressionContext`.
* `AsyncExpressionFunction` now returns `Task<object?>` and receives `FunctionEventArgs`. `AsyncExpressionParameter` was removed; dynamic parameters now use `ExpressionParameter`.
